%% Phase Coherence Analysis for MDMA Study
% This script performs phase coherence analysis on MDMA vs Placebo fMRI data
...
%
% Required Data Files:
% - MDMA_TS_FC.mat: Contains time series data for MDMA and Placebo conditions
...
try
    ...
    error('Required data files not found. Please ensure MDMA_TS_FC.mat and Structural.mat are in the path.');
end

% Concatenate MDMA and Placebo data
% Note: Using specific field names for MDMA study (TS_MDMA2 and TS_PCB2)
valid_subjects = [1:6 8:13 15:16 18:21];  % Subjects with valid data
MDMA_data = cellfun(@(x) x.TS_MDMA2, num2cell(DataCorrel(valid_subjects)), 'UniformOutput', false);
PCB_data = cellfun(@(x) x.TS_PCB2, num2cell(DataCorrel(valid_subjects)), 'UniformOutput', false);
MDMA_cat = horzcat(MDMA_data{:});
PCB_cat = horzcat(PCB_data{:});
example_data = horzcat(MDMA_cat, PCB_cat);
...
mdma_length = length(MDMA_cat);
for bst = 1:N_STATES
    rate(bst) = sum(cidx_Pha==I(bst))/(L-2*T_SHIFT);
    ratea(bst) = sum(cidx_Pha(1:mdma_length)==I(bst))/(L-2*T_SHIFT);
    rateb(bst) = sum(cidx_Pha((mdma_length+1):end)==I(bst))/(L-2*T_SHIFT);
end
...
n_subjects = length(valid_subjects);
for k = 1:n_subjects
    subplot(ceil(n_subjects/3), 3, k)
    imagesc(corrcoef(DataCorrel(valid_subjects(k)).TS_MDMA2'))
    colormap(jet)
    axis square
    title(sprintf('Subject %d - MDMA', valid_subjects(k)))
    colorbar
end
