%% Phase Coherence Analysis for Ketamine Study
% This script performs phase coherence analysis on Ketamine vs Placebo fMRI data
...
%
% Required Data Files:
% - KETA_TS_FC.mat: Contains time series data for Ketamine and Placebo conditions
...
try
    load('KETA_TS_FC.mat', 'DataCorrel');
    load('Structural.mat', 'SC');
catch ME
    error('Required data files not found. Please ensure KETA_TS_FC.mat and Structural.mat are in the path.');
end

% Concatenate Ketamine and Placebo data
KETA_data = cellfun(@(x) x.KET_TS, num2cell(DataCorrel), 'UniformOutput', false);
PCB_data = cellfun(@(x) x.PCB_TS, num2cell(DataCorrel), 'UniformOutput', false);
KETA_cat = horzcat(KETA_data{:});
PCB_cat = horzcat(PCB_data{:});
example_data = horzcat(KETA_cat, PCB_cat);
...
for bst = 1:N_STATES
    ...
    ratea(bst) = sum(cidx_Pha(1:length(KETA_cat))==I(bst))/(L-2*T_SHIFT);
    rateb(bst) = sum(cidx_Pha((length(KETA_cat)+1):end)==I(bst))/(L-2*T_SHIFT);
end
